...
public:
    ...
    int solve(int m , int n,vector<vi> grid,vector<vi> dp){
        if(m == 0 && n == 0)
            return grid[0][0];
    if(m > 0 && n > 0){
        return dp[m][n] = min(grid[m][n] + solve(m-1,n,grid,dp),grid[m][n] + solve(m,n-1,grid,dp));
    }else if(m < 1)
            return dp[m][n] = grid[m][n] + solve(m,n-1,grid,dp);
        else
            return dp[m][n] = grid[m][n] + solve(m-1,n,grid,dp);
